...
function google_business_reviews_rating_admin(popstate) {
	...
	if (jQuery('.section', '#wpbody-content').length) {
		...
		if (jQuery('#update, #welcome-update').length) {
			if (jQuery('#update-billing').length && jQuery('#update').length && jQuery('#update').is(':visible')) {
				if (jQuery('#update-billing').is(':hidden') && (typeof jQuery('#update').val() == 'number' || typeof jQuery('#update').val() == 'string' && jQuery('#update').val().match(/^\d+$/))) {
					jQuery('#update-billing').show();
				}
				else if (jQuery('#update-billing').is(':visible') && (jQuery('#update').val() == null || !jQuery('#update').val().length || typeof jQuery('#update').val() != 'number' && typeof jQuery('#update').val() == 'string' && !jQuery('#update').val().match(/^\d+$/))) {
					jQuery('#update-billing').hide();
				}
			}
			
			if (jQuery('#welcome-update-billing').length && jQuery('#welcome-update').length && jQuery('#welcome-update').is(':visible')) {
				if (jQuery('#welcome-update-billing').is(':hidden') && (typeof jQuery('#welcome-update').val() == 'number' || typeof jQuery('#welcome-update').val() == 'string' && jQuery('#welcome-update').val().match(/^\d+$/))) {
					jQuery('#welcome-update-billing').show();
				}
				else if (jQuery('#welcome-update-billing').is(':visible') && (jQuery('#welcome-update').val() == null || !jQuery('#welcome-update').val().length || typeof jQuery('#welcome-update').val() != 'number' && typeof jQuery('#welcome-update').val() == 'string' && !jQuery('#welcome-update').val().match(/^\d+$/))) {
					jQuery('#welcome-update-billing').hide();
				}
			}
		}
	}
	...
	if (jQuery('#google-business-reviews-rating-settings').length && jQuery('.section', '#wpbody-content').length) {
		...
		jQuery('#review-limit, #review-limit-hide, #review-limit-set, #review-limit-all', '#wpbody-content').on('change', function() {
			...
			if (jQuery(this).is('#review-limit')) {
				...
				if (typeof review_limit != 'number') {
					...
					else if (jQuery('#theme-recommendation-bubble').is(':hidden') && jQuery('#reviews-theme').val().match(/\bbubble\b/i)) {
					...
					else if (jQuery('#theme-recommendation-bubble').is(':visible') && jQuery('#reviews-theme').val().match(/\bbubble\b/i)) {
					...
					if (jQuery('#reviews-theme').val().match(/\bcolumns\b/i)) {
						if (jQuery('#theme-recommendation-columns').is(':hidden') && review_limit%theme_columns > 0) {						
							if (jQuery('#reviews-theme').val().match(/\bbubble\b/i)) {
								jQuery('#theme-recommendation-bubble').slideUp(300);
							}
					...
					else if (jQuery('#reviews-theme').val().match(/\bbubble\b/i)) {
						jQuery('#theme-recommendation-bubble').slideDown(300);
					}
				}
			}
		...
		if (jQuery('#update, #welcome-update').length) {
			...
			jQuery('#update, #welcome-update').on('change', function() {
				if (jQuery('#update-billing').length && jQuery('#update').length && jQuery('#update').is(':visible')) {
					if (jQuery('#update-billing').is(':hidden') && typeof jQuery('#update').val() == 'number' || typeof jQuery('#update').val() == 'string' && jQuery('#update').val().match(/^\d+$/)) {
						jQuery('#update-billing').slideDown(300);
					}
					else if (jQuery('#update-billing').is(':visible') && (jQuery('#update').val() == null || !jQuery('#update').val().length || typeof jQuery('#update').val() != 'number' && typeof jQuery('#update').val() == 'string' && !jQuery('#update').val().match(/^\d+$/))) {
						jQuery('#update-billing').slideUp(300);
					}
				}
				
				if (jQuery('#welcome-update-billing').length && jQuery('#welcome-update').length && jQuery('#welcome-update').is(':visible')) {
					if (jQuery('#welcome-update-billing').is(':hidden') && typeof jQuery('#welcome-update').val() == 'number' || typeof jQuery('#welcome-update').val() == 'string' && jQuery('#welcome-update').val().match(/^\d+$/)) {
						jQuery('#welcome-update-billing').slideDown(300);
					}
					else if (jQuery('#welcome-update-billing').is(':visible') && (jQuery('#welcome-update').val() == null || !jQuery('#welcome-update').val().length || typeof jQuery('#welcome-update').val() != 'number' && typeof jQuery('#welcome-update').val() == 'string' && !jQuery('#welcome-update').val().match(/^\d+$/))) {
						jQuery('#welcome-update-billing').slideUp(300);
					}
				}
			});
		}
		...
		jQuery('#google-credentials-help, #welcome-google-credentials-help').on('click', function(event) {
			event.preventDefault();
			if (jQuery(this).attr('id').match(/welcome/i)) {
	...
	if (jQuery('#html-import').length) {
		...
		jQuery('#html-import, #import-process-button').on('change blur click', function(event) {
			...
			e = (jQuery('.Svr5cf[jslog]', '#html-import-input').length) ? jQuery('.Svr5cf[jslog]', '#html-import-input') : ((jQuery('.WMbnJf', '#html-import-input').length) ? jQuery('.WMbnJf', '#html-import-input') : null);
					...
					for (j = 0; j < 2; j++) {
						for (i in reviews) {
							...
							if (!existing && j == 0 || existing_show && existing && j == 1) {
								date_temp = null;
								time_unit = (review.relative_time_description.match(/^([\d]{1,3})\s+[^\s]+\s+\w+/i)) ? parseInt(review.relative_time_description.replace(/^.*(\d{1,3}).*$/i, '$1')) : 1;
								...
								if (date_temp == null && language != null && language.match(/^(?:de|es|fr|it).*$/i)) {
						...
						jQuery(this).on('change', function() {
							...
							if (jQuery(this).is(':checkbox')) {
								if (jQuery('#review-import-select-all').is(':checked')) {
									jQuery('#review-import-select-all').removeProp('checked');
								}
