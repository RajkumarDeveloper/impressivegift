	...
	public function __construct()
	{
		...
		if (!$this->translation_exists())
		{
			...
			case 'it':
				...
				$this->relative_times['days']['text'] = 'due giorni fa';
...
';
		...
		case 'rating_count':
			...
			{
				$ret = $this->reviews_count($place_id);
				break;
			}
		...
		case 'rating_count_rounded':
			...
			if (isset($this->reviews) && is_array($this->reviews) && !empty($this->reviews))
			{
				$ret = (function_exists('number_format_i18n')) ? number_format_i18n($this->reviews_count($place_id), 0) : number_format($this->reviews_count($place_id), 0);
				break;
			}
	...
	public function set_data($force = NULL, $api_key = NULL, $place_id = NULL)
	{
		...
		if (defined('XMLRPC_REQUEST') && XMLRPC_REQUEST || ((!is_bool($force) || !$force) && defined('DOING_CRON') && DOING_CRON) || isset($_POST['action']) && is_string($_POST['action']) && preg_match('/^heartbeat$/i', $_POST['action']) || isset($_POST['log']) && $_POST['log'] != NULL)
		{
			return FALSE;
		}
...
';
		...
		if ($force || $recheck)
		{
			...
			if (array_key_exists($place_set_key, $this->places) && (!isset($this->result['result']['name']) || isset($this->result['result']['name']) && $this->result['result']['name'] == NULL))
			{
...
';
	...
	public function get_relative_time_description($time, $fallback = NULL, $use_fallback = FALSE)
	{
		...
		foreach ($this->relative_times as $a)
		{
	...
	public function wp_display($atts = NULL, $content = NULL, $shortcode = NULL)
	{
		...
		case 'google_reviews':
			...
			. ((is_string($color_scheme)) ? ' ' . $color_scheme : '')
			. (($stylesheet && is_string($stars)) ? (($stars == 'html' || $stars == 'css') ? ' stars-' . $stars : ' stars-color') : '')
			. (($stylesheet && is_string($stars_gray)) ? (($stars_gray == 'html' || $stars_gray == 'css') ? ' stars-' . $stars_gray : ' stars-gray-color') : '')
			. ((is_string($link)) ? ' link' : '')
...
';
				...
				if ($count)
				{
					$review_word = (count($review_word) == 2 && intval($rating_count_rounded) != 1) ? $review_word[1] : $review_word[0];
					$html .= (($link != $reviews_url) ? '<a href="' . esc_attr($reviews_url). '" target="_blank"' . (($rel != NULL) ? ' rel="' . esc_attr($rel) . '"' : '') . ' class="count">' : '<span class="count">');
					...
					foreach ($this->reviews_filtered as $i => $a)
					{
						...
						if ($author_name != NULL && $name_format != NULL && !empty($name_format_match))
						{
							...
							if (count($author_name_array) == 1 || $author_name_first || $author_name_last || $author_name_first_initials || $author_name_last_initials)
							{
								...
								{
									$author_name = ($author_name_first) ? $author_name_array[0] : strtoupper(substr($author_name_array[0], 0, 1) . (($author_name_dot) ? '.' : ''));
								...
								else
								{
									...
									$author_name .= ($author_name_last) ? end($author_name_array) : ' ' . strtoupper(substr(end($author_name_array), 0, 1) . (($author_name_dot) ? '.' : ''));
								}
							}
							else
							{
								$author_name = ($author_name_last) ? end($author_name_array) : strtoupper(substr($author_name_array[0], 0, 1) . (($author_name_dot) ? '.' : '') . (($author_name_space) ? ' ' : '') . substr(end($author_name_array), 0, 1) . (($author_name_dot) ? '.' : ''));
							}
						...
						$html .= '		<' . $html_tags[4] . ' class="' . esc_attr('rating-' . $a['rating']) . ((is_bool($avatar) && !$avatar) ? ' no-avatar' : '') . (($review_item_text_first) ? ' text-first' : '') . (($review_item_inline) ? ' inline' : '') . (($review_item_author_switch) ? ' author-switch' : '') . '">
';
						if (!$review_item_text_first)
						{
							if (!$review_item_author_switch)
							{
								...
								{
									$html .= '			<span class="author-avatar">' . ((isset($a['author_url']) && $a['author_url'] != NULL) ? '<a href="' . esc_attr($a['author_url']) . '" target="_blank"' . (($rel != NULL) ? ' rel="' . esc_attr($rel) . '"' : '') . '>' : '') . ((isset($a['profile_photo_url']) && $a['profile_photo_url'] != NULL) ? '<img src="' . esc_attr((is_string($avatar)) ? $avatar : $a['profile_photo_url']) . '" alt="Avatar">' : '—') . ((isset($a['author_url']) && $a['author_url'] != NULL) ? '</a>' : '') . '</span>
';
								}
								
								if ($author_name != NULL)
								{
									$html .= '			<span class="author-name">' . ((isset($a['author_url']) && $a['author_url'] != NULL) ? '<a href="' . esc_attr($a['author_url']) . '" target="_blank"' . (($rel != NULL) ? ' rel="' . esc_attr($rel) . '"' : '') . '>' : '') . esc_html($author_name) . ((isset($a['author_url']) && $a['author_url'] != NULL) ? '</a>' : '') . '</span>
';
								}
							}
...
';
						...
						if ($review_item_text_first)
						{
							if (!$review_item_author_switch)
							{
								if ((is_bool($avatar) && $avatar || is_string($avatar)) || (!is_string($theme) || is_string($theme) && !preg_match('/^badge(?:\s+.+)?$/', $theme)))
								{
									$html .= '			<span class="author-avatar"><a href="' . esc_attr($a['author_url']) . '" target="_blank"' . (($rel != NULL) ? ' rel="' . esc_attr($rel) . '"' : '') . '>' . (($a['profile_photo_url'] != NULL) ? '<img src="' . esc_attr((is_string($avatar)) ? $avatar : $a['profile_photo_url']) . '" alt="Avatar">' : '—') . '</a></span>
';
								}
								
								if ($author_name != NULL)
								{
									$html .= '			<span class="author-name"><a href="' . esc_attr($a['author_url']) . '" target="_blank"' . (($rel != NULL) ? ' rel="' . esc_attr($rel) . '"' : '') . '>' . esc_html($author_name) . '</a></span>
';
								}
							}
...
';
	...
	public function translation_exists($loose = FALSE)
	{
		...
		if ($loose)
		{
			return (preg_match('/^(?:(?:de|en|es|f|it).*)?$/', get_option('WPLANG')));
		}
