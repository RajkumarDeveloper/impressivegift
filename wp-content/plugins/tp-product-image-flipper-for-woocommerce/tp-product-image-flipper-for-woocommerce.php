 ...
 */
define( 'TP_PRODUCT_IMAGE_FLIPPER_FOR_WOOCOMMERCE_VERSION', '1.0.2' );
        ...
        if($get_image_id){
            ...
            if($get_gallery_image_ids){
                $output = '<div class="tp-image-wrapper">';
                    ...
                    //wp_dbug($image_url_bottom);
                    //wp_dbug($image_url_top);
                    $output .= '<img class="tp-image" src="'.$image_url_top.'" />';
                    //$output .= '<img class="bottom" width="300" height="300" src="'.$image_url_bottom[0].'" />';
                    $output .= '<img class="tp-image-hover" src="'.$image_url_bottom[0].'" />';
                            
                $output .= '</div>';
                
            }
            else{
                $output = '<div class="tp-image-wrapper"><img class="image" src="'.$image_url_top.'" /></div>';
            }
